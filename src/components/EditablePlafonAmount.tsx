...
import { updateTotalPlafon } from "@/app/actions/pengajuan";
...
export function EditablePlafonAmount({
  plafonId,
  initialValue,
}: {
  plafonId: string;
  ...
  const handleBlur = () => {
    ...
    startTransition(async () => {
      try {
        ...
        formData.append("totalPlafon", numValue.toString());
        
        await updateTotalPlafon(formData);
        router.refresh();
      } catch (error) {
        console.error("Failed to update plafon", error);
        setValue(initialValue.toString());
      }
    });
  };
  ...
  return (
    <div
      ...
      title="Klik untuk mengubah budget Plafon"
    >
