...
function FundRequestCanvas({
  ...
  function buildPrintData(): FundRequestPrintData {
    return {
      id: item.id,
  ...
  async function handleDownloadPdf() {
    ...
    try {
      const doc = await createFundRequestPdf(buildPrintData(), fields.tipePengajuan || "NON KASBON", signatures);
      ...
      >
        ...
        <div className="flex flex-col justify-end gap-3 border-t border-slate-100 px-6 py-4 sm:flex-row">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-purple-200 bg-white px-6 py-2.5 font-semibold text-purple-700 shadow-sm transition-colors hover:bg-purple-50 disabled:cursor-wait disabled:opacity-60"
          >
            <Printer className="h-4 w-4" />
            {isGenerating ? "Membuat PDF..." : isPending ? "Menyimpan..." : "Cetak"}
          </button>
          <button
            type="button"
